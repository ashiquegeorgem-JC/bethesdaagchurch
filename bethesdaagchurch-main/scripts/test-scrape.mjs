...
async function testScrape() {
  const res = await fetch('https://www.youtube.com/@Bethesda_AG/videos', {
    headers: {
      'User-Agent':
  ...
  // Look for ytInitialData JSON inside HTML
  ...
  if (match) {
        ...
        if (metaRows) {
          for (const row of metaRows) {
            for (const part of (row.metadataParts || [])) {
              ...
              if (text.includes('ago') || text.includes('year') || text.includes('month') || text.includes('week') || text.includes('day') || text.includes('Streamed')) {
                publishedAt = text;
              }
            }
          }
    ...
    walk(ytData);
    console.log(`Extracted ${videos.length} videos from initial HTML!`);
    videos.slice(0, 5).forEach((v, i) => console.log(`${i+1}. [${v.videoId}] ${v.title} (${v.publishedAt})`));
  } else {
    console.log('ytInitialData regex did not match, testing alternate match...');
    const match2 = html.match(/ytInitialData"\s*:\s*({[\s\S]*?})\s*;\s*</);
