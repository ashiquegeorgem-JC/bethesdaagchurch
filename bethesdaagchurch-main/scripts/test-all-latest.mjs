async function fetchAllLatest() {
  const [streamsRes, videosRes] = await Promise.all([
    fetch('https://www.youtube.com/@Bethesda_AG/streams', {
      headers: {
        'User-Agent':
    ...
    fetch('https://www.youtube.com/@Bethesda_AG/videos', {
      headers: {
        'User-Agent':
          'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36',
        'Accept-Language': 'en-US,en;q=0.9',
  ...
  extract(await streamsRes.text());
  extract(await videosRes.text());

  console.log(`TOTAL UNIQUE RECENT MESSAGES & LIVE STREAMS: ${videos.length}`);
  videos.slice(0, 10).forEach((v, i) => console.log(`${i+1}. [${v.videoId}] ${v.title} (${v.publishedAt})`));
}
