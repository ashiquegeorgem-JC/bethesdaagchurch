      ...
      <main className="min-h-screen bg-background pb-20">
        ...
        <section className="relative bg-navy text-ivory text-center py-20 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-4 relative z-10">
            <span className="text-overline font-semibold uppercase tracking-[0.25em] text-gold">YOUTUBE CHANNEL</span>
            ...
            <p className="text-body-lg text-ivory/90 max-w-2xl mx-auto leading-relaxed mb-6">
              Watch all our live services, Sunday messages, and special events on our official YouTube channel — <span className="text-gold font-semibold">@Bethesda_AG</span>.
            </p>
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <a href="https://www.youtube.com/@Bethesda_AG/live" target="_blank" rel="noopener noreferrer">
                <Button variant="outline" size="lg">
                  <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse mr-2 inline-block" />
                  WATCH LIVE
                </Button>
              </a>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-10">
          <div className="flex items-center justify-between mb-8">
            <h2 className="font-display font-bold text-2xl text-charcoal dark:text-ivory">Latest Messages</h2>
            <a href="https://www.youtube.com/@Bethesda_AG/videos" target="_blank" rel="noopener noreferrer" className="text-sm text-gold hover:underline font-semibold">
              View all on YouTube →
            </a>
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="bg-card border border-sand rounded-2xl overflow-hidden animate-pulse">
            ...
            <div className="text-center py-16 bg-card border border-border rounded-2xl">
              <p className="text-muted-foreground mb-4">Visit our YouTube channel for all messages.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {videos.map((video, idx) => (
                <div key={video.id} className="bg-card border border-sand rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 flex flex-col">
                  <a
                    href={video.url}
                    target="_blank"
                  ...
                  >
                    ...
                    {idx === 0 && (
                      <span className="absolute top-3 left-3 bg-gold text-white text-xs font-bold px-2.5 py-1 rounded-full">LATEST</span>
                    )}
                  </a>
                  <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                    <div>
                      <p className="text-xs text-gold font-semibold mb-1">Bethesda AG Church</p>
                      <h3 className="font-display font-bold text-lg text-foreground leading-snug line-clamp-2">{video.title}</h3>
                      {video.publishedAt && (
