...
export default function GiveWays() {
  ...
  return (
    <section
      ...
      aria-labelledby="give-ways-heading"
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {ways?.map((way, i) => (
            <div
              ...
              className={`animate-on-scroll rounded-3xl p-8 border card-hover-lift relative overflow-hidden ${
                way?.highlight
                  ? 'bg-church-green border-church-gold/30 text-white'
                  : 'glass-card-ivory border-church-gold/20'
              }`}
            ...
            >
              ...
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center mb-6 ${way?.highlight ? 'bg-church-gold/20' : 'bg-church-sand'}`}
              >
                <svg
                  className={`w-6 h-6 ${way?.highlight ? 'text-church-gold' : 'text-church-green'}`}
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="1.5"
                  viewBox="0 0 24 24"
              ...
              <h3
                className={`font-display text-2xl font-bold mb-3 ${way?.highlight ? 'text-white' : 'text-church-green'}`}
              >
                {way?.title}
              </h3>
              <p
                className={`text-sm leading-relaxed mb-4 ${way?.highlight ? 'text-white/70' : 'text-church-bronze'}`}
              >
                {way?.desc}
              </p>
              <p
                className={`text-xs italic mb-6 ${way?.highlight ? 'text-white/40' : 'text-church-bronze/50'}`}
              >
                {way?.note}
              </p>
              <button
                className={`w-full py-3 rounded-xl font-semibold text-sm transition-all duration-300 ${
                  way?.highlight ? 'btn-gold text-church-green' : 'btn-primary-church text-white'
                }`}
              >
                {way?.action}
              </button>
        ...
        <div className="mt-10 text-center animate-on-scroll">
          <p className="text-church-bronze/70 text-sm">
            <svg
              className="w-4 h-4 inline-block mr-1 text-gold"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth="2"
            ...
            All online transactions are secure and encrypted. Tax-deductible receipts are provided
            for all donations.
          </p>
        </div>
      </div>
