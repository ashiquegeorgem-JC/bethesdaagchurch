    ...
    <>
      ...
      <main className="min-h-screen bg-background pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
          <Breadcrumbs items={[{ label: 'Community', href: '/community' }, { label: 'Small Groups' }]} />
        </div>
        ...
        <section className="relative bg-navy text-ivory text-center py-20 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-4 relative z-10">
            <span className="text-overline font-semibold uppercase tracking-[0.25em] text-gold">SMALL GROUPS FINDER</span>
            ...
            <p className="text-body-lg text-ivory/90 max-w-2xl mx-auto leading-relaxed">
              Small groups are where a big church feels like family. Connect with believers in your neighborhood for Bible study, prayer, and encouragement.
            </p>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-10">
          <div className="bg-card border border-border p-6 rounded-2xl shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              <select value={dayFilter} onChange={(e) => setDayFilter(e.target.value)} className="px-4 py-2.5 bg-input border border-border rounded-xl text-xs font-semibold">
              ...
              <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="px-4 py-2.5 bg-input border border-border rounded-xl text-xs font-semibold">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredGroups.map((group) => (
              <div key={group.id} className="bg-card border border-border p-6 rounded-3xl shadow-sm hover:shadow-xl hover:border-gold/40 transition-all duration-300 flex flex-col justify-between space-y-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Badge variant="gold">{group.type}</Badge>
                    <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400">{group.spotsAvailable} Spots Open</span>
                  </div>
                  <h3 className="font-display font-bold text-heading-md text-foreground">{group.name}</h3>
                ...
                <div className="bg-input p-4 rounded-2xl text-xs text-foreground space-y-1.5 border border-border">
                  <p><strong>Day:</strong> {group.day}s at {group.time}</p>
                  <p><strong>Location:</strong> {group.location}</p>
                  <p><strong>Leader:</strong> {group.leader}</p>
                  <p><strong>Target:</strong> {group.ageGroup}</p>
                </div>

                <Button variant="gold" size="sm" onClick={() => handleJoinGroup(group.name)} className="w-full">
                  JOIN GROUP
                </Button>
              </div>
            ))}
