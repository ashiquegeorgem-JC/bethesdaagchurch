    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          variant="gold"
          size="sm"
          onClick={() => addToast({ type: 'info', title: 'Add Ministry' })}
        >
          + ADD MINISTRY
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {minList.map((m) => (
          <div
            key={m.id}
            className="bg-navy border border-gold/20 p-6 rounded-2xl space-y-3 shadow-xl"
          >
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-white/10">
              <button
                onClick={() => addToast({ type: 'info', title: `Editing ${m.name}` })}
                className="px-2 py-1 bg-white/10 text-xs rounded hover:bg-white/20"
              >
                Edit
              </button>
            </div>
