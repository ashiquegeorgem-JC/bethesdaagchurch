    ...
    <>
      ...
      <main className="min-h-screen bg-background pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
          <Breadcrumbs
            items={[{ label: 'Messages', href: '/messages' }, { label: sermon.title }]}
          />
        </div>
            ...
            {isPlaying ? (
              ...
              <>
                ...
                <span className="absolute bottom-4 left-4 text-xs font-semibold text-ivory bg-black/70 backdrop-blur-md px-3.5 py-1.5 rounded-lg flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /> YouTube HD
                  Stream • {sermon.duration}
                </span>
              </>
            )}
          </div>
          ...
          <div className="bg-card border border-sand p-8 sm:p-10 rounded-3xl shadow-card space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <h1 className="font-display font-bold text-display-sm sm:text-display-md text-charcoal dark:text-ivory">
                  {sermon.title}
                </h1>
                <p className="text-body-sm text-gold font-semibold">
                  {sermon.speaker} • {sermon.date} • {sermon.bibleRef}
                </p>
              </div>
            ...
            <div>
              <h3 className="font-display font-bold text-2xl text-charcoal dark:text-ivory mb-2">
                Description
              </h3>
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <h2 className="font-display font-bold text-3xl text-charcoal dark:text-ivory mb-6">
            Related Messages
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {related.map((rel) => (
              <div
                key={rel.id}
                className="bg-card border border-sand p-5 rounded-2xl flex items-center gap-4 hover:border-gold/40 transition-colors shadow-card"
              >
                ...
                <div className="space-y-1">
                  ...
                  <h4 className="font-display font-bold text-base text-charcoal dark:text-ivory line-clamp-1">
                    {rel.title}
                  </h4>
                  <Link
                    href={`/messages/${rel.slug}`}
