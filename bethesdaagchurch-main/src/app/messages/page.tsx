    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 bg-charcoal/30 group-hover:bg-charcoal/10 transition-colors flex items-center justify-center">
          <div
            className={`p-3 rounded-full bg-gold text-white transition-transform duration-200 ${hovered ? 'scale-110' : 'scale-100'} shadow-lg`}
          >
      ...
      <div
        className={`p-6 sm:p-8 flex flex-col justify-between flex-1 ${featured ? 'lg:p-10' : ''}`}
      >
        <div className="space-y-3">
          ...
          <h3
            className={`font-display font-bold text-charcoal dark:text-ivory leading-tight ${featured ? 'text-2xl sm:text-3xl' : 'text-xl'}`}
          >
            {video.title}
          </h3>
          {video.publishedAt && (
...
function SkeletonCard({ featured = false }: { featured?: boolean }) {
  return (
    <div
      className={`bg-card border border-sand rounded-3xl overflow-hidden animate-pulse flex flex-col ${featured ? 'lg:flex-row' : ''}`}
    >
...
export default function MessagesPage() {
  const { videos: appVideos, videosLoading } = useApp();
        ...
        <section className="relative bg-navy text-ivory text-center py-20 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-4 relative z-10">
            ...
            <p className="text-body-lg text-ivory/90 max-w-2xl mx-auto leading-relaxed mb-6">
              All messages are streamed live and uploaded directly from our official YouTube channel
              — <span className="text-gold font-semibold">@Bethesda_AG</span>.
            </p>
            <a
                ...
                (viewMode === 'grid' ? (
                  ...
                  <div className="space-y-4">
                    {rest.map((video) => (
                      <div
                        key={video.id}
                        className="bg-card border border-border p-4 rounded-2xl flex flex-col sm:flex-row items-center gap-5 hover:border-gold/40 transition-colors"
                      >
                        <a
                          href={video.url}
                          target="_blank"
